...
const STATUS_CONFIG: Record<string, { dot: string; text: string }> = {
  ...
  unattached: { dot: "bg-orange-500", text: "text-orange-600" },
  READY: { dot: "bg-warning", text: "text-warning" },
};

const TYPE_CONFIG: Record<string, { label: string; color: string }> = {
  compute: { label: "VM", color: "bg-blue-900/50 text-blue-600 border-blue-200" },
  cloud_function: { label: "FN", color: "bg-violet-900/50 text-violet-600 border-violet-200" },
  gcs: { label: "GCS", color: "bg-emerald-900/50 text-emerald-600 border-emerald-200" },
  disk: { label: "DISK", color: "bg-amber-900/50 text-amber-400 border-amber-800/50" },
  cloud_sql: { label: "SQL", color: "bg-red-900/50 text-red-600 border-red-200" },
};
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-border">
            ...
            <th className="text-left py-2.5 px-3 text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Monthly</th>
            <th className="text-left py-2.5 px-3 text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Last Seen</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-border-subtle">
          {resources.map((r) => {
            const typeConf = TYPE_CONFIG[r.resource_type] || { label: r.resource_type, color: "bg-slate-100 text-slate-600" };
            ...
            return (
              <tr
                ...
                className={`hover:bg-surface-overlay/30 transition-colors ${onSelectResource ? "cursor-pointer" : ""}`}
              >
